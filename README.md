...
2. Scope
In-Scope:
Functional testing of booking creation, modification, and deletion.

API testing for endpoints related to bookings, rooms, authentication, reporting, and messaging.
...
Ensure that sensitive data, such as passwords, are encrypted.
...
Tools: Postman for API testing, JMeter for performance testing, and OWASP ZAP for security testing.
...
Severity: High
Steps to Reproduce:
Navigate to the room booking calendar.
Attempt to select a past date for the booking.
...
Severity: High
Steps to Reproduce:
Attempt to book a room without filling in all required fields.
Submit the form.
...
Book a room without signing in or providing any user credentials.
...
Try to select a date by clicking on it in the calendar.
...
Severity: Medium
Steps to Reproduce:
Submit the booking form or the contact form with invalid data (e.g., invalid email, missing fields).
...
Severity: High
Steps to Reproduce:
Enter an email without a ".com" in the email field.
Enter alphanumeric or special characters in the phone number field.
...
Severity: Low
Steps to Reproduce:
Navigate to a room listing page.
Check the font style of the room title.
...
Look for the title of the contact form section.
...
Severity: High
Steps to Reproduce:
Log into the admin portal and navigate to the Rooms tab.
Create a room with a specific name.
Create another room with the same name.
...
Severity: High
Steps to Reproduce:
Navigate to the admin portal and go to the Rooms tab.
Enter a room number that already exists.
Try to add an image to the room.
...
Enter invalid credentials (e.g., incorrect username/password).
...
Severity: Medium
Steps to Reproduce:
Fill out the contact form with an extremely large string in the "Name" field.
Submit the form.
...
Expected Result: The copyright year should be updated to the current year (e.g., 2025).

15. Bug Title: Overlapping Booking Dates
Description: There are overlapping dates available when booking a room.

Severity: High
Steps to Reproduce:
Select a room and try to book it for a specific date range.
Try to book the same room for overlapping dates.
Actual Result: The system allows overlapping date ranges for the same room.
Expected Result: The system should prevent booking overlapping dates for the same room.